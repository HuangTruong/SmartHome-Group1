...
function applyAutomation() {
  ...
  // Đèn tự động
  if (modes.light === 'AUTO') {
    if (light < thresholds.lightOn  && !devices.light) { devices.light = true;  addLog('auto', '🌙 Trời tối → bật đèn.'); }
    else if (light > thresholds.lightOff &&  devices.light) { devices.light = false; addLog('auto', '☀️ Trời sáng → tắt đèn.'); }
  }

  // Quạt tự động
  if (modes.fan === 'AUTO') {
    if (temperature > thresholds.fanOn  && !devices.fan) { devices.fan = true;  addLog('auto', '🔥 Nhiệt độ cao → bật quạt.'); }
    else if (temperature < thresholds.fanOff &&  devices.fan) { devices.fan = false; addLog('auto', '❄️ Nhiệt độ thấp → tắt quạt.'); }
  }
...
app.post('/api/control/light', (req, res) => {
  ...
  if (action === 'AUTO') { state.modes.light = 'AUTO'; addLog('manual', 'Đèn chuyển AUTO'); }
  else { state.modes.light = 'MANUAL'; state.devices.light = action === 'ON'; addLog('manual', `User ${action} đèn`); }
...
app.post('/api/control/fan', (req, res) => {
  ...
  if (action === 'AUTO') { state.modes.fan = 'AUTO'; }
  else { state.modes.fan = 'MANUAL'; state.devices.fan = action === 'ON'; }
